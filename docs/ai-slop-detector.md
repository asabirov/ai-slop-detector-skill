...
skill page and not the code, so clone this repository where Claude Code looks for
personal skills:
...
The rules live once, in `asabirov/ai-slop-detector-skill`, and every consumer reaches
them there: a CI job through `npx`, a session through a clone. The
`apliteni:ai-slop-detector` skill inside `apliteni/claude-apliteni-plugin` is a pointer
to that repository, regenerated by a GitHub Action, and carries no rules of its own.
...
version `4.0.0`. Either route above replaces both copies, and neither leaves a copy to
prune.
