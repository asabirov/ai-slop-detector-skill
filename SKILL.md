...
3. **Give presentation a purpose.** Flag decoration that misleads or obstructs reading, such as fake addresses, numbers implying a nonexistent sequence, or labels with no information. Fonts, colors, punctuation, and layout alone are not enough. Respect intentional design. Flag a selected dropdown option drawn as a filled accent-colour box with a matching rounded border, especially beside a search field that already has a strong focus outline: the competing frames make selection look like another focused control. Prefer a quiet row with a checkmark or restrained fill; keep keyboard focus visible and distinct from selection. A focus ring on the control that actually has focus is not this pattern.
